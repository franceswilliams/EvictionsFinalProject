...
line_graph_3 %>% 
  group_by(neighborhood, year) %>% 
   ungroup() %>% 
    plot_ly(x=~year, y=~wt_eviction_rate, group=~neighborhood, type="scatter", color=~neighborhood, mode="lines+markers") %>% 
    layout(title="Eviction Rate (%) Over Time by Brooklyn Neighborhood")

# This preliminary plotly looks extremely busy....attempting to incorporate Shiny to make it not so busy :)
