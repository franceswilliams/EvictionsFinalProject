...
Bar chart of eviction rate/filing discrepancy
```{r}
...
#HOW TO WTICH RATE AND RATE DISCREP ON THE BAR
...
barchart_evictions = neighborhood_data %>%
  top_n(15, wt_eviction_rate) %>% 
  mutate(neighborhood = fct_reorder(neighborhood, wt_eviction_rate)) %>% 
  ggplot(aes(x = neighborhood, y = wt_eviction_rate)) +
...
bar_chart_eviction_discrep = neighborhood_data %>%
  ...
  ggplot(aes(x = neighborhood, y = wt_eviction_rate_discrep)) +
    geom_bar(stat = "identity") +
    coord_flip()
...
bar_chart_pct_nonwhite = neighborhood_data %>%
  ...
  ggplot(aes(x = neighborhood, y = wt_pct_nonwhite)) +
    geom_bar(stat = "identity") +
    coord_flip()
...
bar_chart_poverty = neighborhood_data %>%
  ...
  ggplot(aes(x = neighborhood, y = wt_povertyrate)) +
    geom_bar(stat = "identity") +
    coord_flip()
