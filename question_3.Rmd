...
## NOTES
  ## include hisp instead of other, since highly correlated
  ## include black not white, since highly correlated
  ## fit two models:
...
## need to include `black` since confounds pct_eng -> evictions relationship
  ...
  ## then include races (minus white and hispanic) + pct_eng (ESL controlling for races)
```
...
step_model = 
  geeglm(formula = evictions ~ offset(log(renter_occupied_households)) + 
           years_since_2010 + 
           black + hisp + aian + asian + nhpi +
           rent_burden + median_gross_rent + density + pct_eng + poverty_rate + median_household_income, 
         family = poisson, 
         data = joined_data_bklyn_nomissing, 
         id = geoid, 
         corstr = "ar1")
